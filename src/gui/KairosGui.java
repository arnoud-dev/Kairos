...
public class KairosGui extends Application {
    ...
    private void switchContent(String page) {
        if (!activePage.isEmpty()) {
            ...
            if (old != null) old.setStyle("-fx-background-color: transparent; -fx-text-fill: white; -fx-font-size: 16px;");
        }

        activePage = page;
        Button activeBtn = sidebarButtons.get(page);
        if (activeBtn != null) activeBtn.setStyle("-fx-background-color: #1abc9c; -fx-text-fill: white; -fx-font-size: 16px;");
